...
const getFacilityTypeId = async (facilityTypes, facilityTypeName) => {
  if (facilityTypeName) {
    const facilityType = facilityTypes.filter(
      (facilityType) => facilityType.facility_type == facilityTypeName,
    )[0];
    if (facilityType) {
  ...
  return faker.random.arrayElement(
    facilityTypes.map((facilityType) => facilityType.id),
  );
};

const getFacilityOwnerId = async (facilityOwners, facilityOwnerName) => {
  if (facilityOwnerName) {
    const facilityOwner = facilityOwners.filter(
      (facilityOwner) => facilityOwner.facility_owner == facilityOwnerName,
    )[0];
    if (facilityOwner) {
  ...
  return faker.random.arrayElement(
    facilityOwners.map((facilityOwner) => facilityOwner.id),
  );
};
...
const formatFacility = async (facility, requiredModels) => {
  const facilityTypeId = await getFacilityTypeId(
    requiredModels.facilityTypes,
    facility['Facility Type'],
  );
  const facilityOwnerId = await getFacilityOwnerId(
    requiredModels.owners,
    facility['Facility Ownership'],
  );
  const districtId = await getDistrictId(
    requiredModels.districts,
    facility['District'],
  );

  const regulatoryStatusId = await getEntityId(
    requiredModels.regulatoryStatuses,
    null,
    null,
  );
  const operationalStatusId = await getEntityId(
    requiredModels.operationalStatuses,
    null,
    null,
  );
  return {
    ...
    facility_code_dhis2:
      facility['DHIS2 CODE'] == null ? null : '' + facility['DHIS2 CODE'],
...
const populateIndependentModels = async (facilities) => {
  const facilityTypes = _.uniq(
    facilities.map((facility) => facility['Facility Type']),
  ).map((facilityType) => ({
    facility_type: facilityType ? facilityType : faker.lorem.word(),
  }));
  const owners = _.uniq(
    facilities.map((facility) => facility['Facility Ownership']),
  ).map((owner) => ({
    facility_owner: owner ? owner : faker.company.companyName(),
  }));

  const districts = _.uniq(
    facilities.map((facility) => facility['District']),
  ).map((district) => ({
...
const populateDependentModels = async () => {
...
const mapDistrictsToZones = async () => {
  const organisationUnits = JSON.parse(
    await fs.readFileSync('dhis2-organisation-units.json', 'utf8'),
  ).organisationUnits.filter(
    (organisationUnit) => organisationUnit.name != 'Central Hospital',
  );
  await server.models.Zone.deleteAll();
  const zones = await server.models.Zone.create(
  ...
  const dhis2DistrictsPlusZones = _.uniqBy(
    _.flatten(
      organisationUnits.map((organisationUnit) => {
        return organisationUnit.children.map((child) => {
          const districtNameSegemented = child.name.split('-');
          const districtName =
            districtNameSegemented.length > 1
              ? districtNameSegemented.slice(0, -1).join(' ')
              : districtNameSegemented[0];
          return {
  ...
  const mhfrDistrictsWithZoneIds = _.flatten(
    mhfrDistricts.map((mhfrDistrict) => {
        ...
        if (dhi2DistrictPlusZone.district_name == mhfrDistrict.district_name) {
          return {
            ...
            zone_id: zones.filter(
              (zone) => zone.zone_name == dhi2DistrictPlusZone.zone_name,
            )[0].id,
          };
        }
      });
  ...
  const districtCodes = _.flatten(
    Object.keys(districtZoneMapping).map((key) => {
  ...
  for (const mhfrDistrictWithZoneId of mhfrDistrictsWithZoneIds) {
    const district = await server.models.District.findOne({
      where: { id: mhfrDistrictWithZoneId.id },
    });
    const districtCode =
      districtCodes.filter(
        (districtCode) =>
          _.trim(_.lowerCase(district.district_name)) ==
          _.trim(_.lowerCase(districtCode.district_name)),
      )[0].district_code || '';
    if (district) {
...
const loadFromFile = async () => {
  ...
  const facilities = _.uniqBy(
    ...
    (facility) =>
      facility['Facility Name'] &&
      facility['Facility Type'] != 'Village Clinic',
  );

  const facs = [];
  const unwantedCodes = [0, -1, -2, -3];
  const dhis2ExistingCodes = [];
  for (const facility of facilities) {
    if (
      unwantedCodes.includes(Number(facility['DHIS2 CODE'])) ||
      dhis2ExistingCodes.includes(facility['DHIS2 CODE'])
    ) {
      facility['DHIS2 CODE'] = null;
    }
    dhis2ExistingCodes.push(facility['DHIS2 CODE']);
    facs.push(facility);
  }
  return facs;
};
...
const populate = async (facilities) => {
  try {
    ...
    const requiredModels = {
      ...
      clientIds: (await server.models.Client.find()).map(
        (client) => client['id'],
      ),
    };
    await server.models.Facility.deleteAll();
    const formattedFacilities = uniqueFacilities.map(
      async (facility) => await formatFacility(facility, requiredModels),
    );
    await console.log('Populating facilities');

    const awaitedFormattedFacilities = await Promise.all(formattedFacilities);
    const savedFacilities = [];
    for (const awaitedFormattedFacility of awaitedFormattedFacilities) {
      savedFacilities.push(
        await server.models.Facility.create(awaitedFormattedFacility),
      );
    }
    // const savedFacilities = await server.models.Facility.create(
    //   await Promise.all(formattedFacilities)
    // );
    ...
    const facilityGeocodeDataRaw = await facilitiesNameWithGeocodes.map(
      (facilityWithGeocodes) => {
        return savedFacilities.map((facility) => {
          if (
            facilityWithGeocodes['facilityName'] == facility['facility_name']
          ) {
    ...
    const facilityGeocodeData = _.flatten(facilityGeocodeDataRaw).filter(
      (fgd) => fgd,
    );
    await independentModelFactory(
      server.models.Geolocation,
      facilityGeocodeData,
    );
