...
module.exports = async facility => {
  ...
  const { contact_person_phone = "", contact_person_email = "" } =
    contactPerson || {};
  const { latitude = "", longitude = "" } = geolocation || {};
  const { facility_code, facility_name, common_name } = facility || {};
  const { postal_address = "", physical_address = "", village = "" } =
    address || {};
  return {
    ...
    address: {
      use: "work",
      type: "both",
      line: [physical_address, postal_address, village]
    },
    endpoint: url
  };
};
