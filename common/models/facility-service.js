...
module.exports = function (Facilityservice) {
  ...
  Facilityservice.saveMany = async (facility_id, service_ids, client_id, cb) => {
    const allFacilityServices = await Facilityservice.find({
      where: { facility_id }
    });

    const facilityServices = service_ids.filter(id => {
      const isExist = allFacilityServices.map(fs => fs.service_id).includes(Number(id))
      return !isExist;
    }).map(id => {
      return {
        service_id: id,
    ...
    if (facilityServices.length > 0) await Facilityservice.create(facilityServices)
  ...
  Facilityservice.hierarchy = async (facility_id, cb) => {
    ...
    const facilityServices = await Facilityservice.find({ where: { facility_id } });
    ...
    const hierarchy = levelOne.map(service => {
      return {
        ...
        serviceType: allServiceTypes
          .filter(st => {
            const sTid = allServices.filter(s => s.id == service.service_id)[0].service_type_id;
            return st.id === sTid
          })[0],
        children: facilityServices.filter(fs => {
          const potentialChildren = allServices.filter(s => s.service_category_id === service.service_id)
          ...
          return {
            ...
            children: facilityServices.filter(fs => {
              const potentialChildren = allServices.filter(s => s.service_category_id === lOneChild.service_id)
              return potentialChildren.map(pc => pc.id).includes(fs.service_id)
            }).map(lTwoChild => {
              return {
                facilityService: lTwoChild,
                service: allServices.filter(s => s.id == lTwoChild.service_id)[0]
              }
            })
          }
        })
