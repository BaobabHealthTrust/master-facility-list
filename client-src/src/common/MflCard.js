  ...
  return (
    <div className="z-depth-2 mlf-w-9">
      <div className="mfl-card-title bg-grey-light">
        <i className="material-icons mfl-icon left mfl-icon-color">{icon}</i>
        {heading}
      </div>
            ...
            {data.map((row, index) => {
              return (
                <tr key={index} className="mfl-card-row">
                  <td className="mfl-summary-subheader">{row[0]}</td>
                  <td className="right-align mfl-summary-subtext">
                    {row[1] ? (
                      row[1]
                    ) : (
                      <i className="material-icons mfl-icon right">check</i>
                    )}
                  </td>
                </tr>
              );
