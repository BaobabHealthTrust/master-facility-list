...
class AdvancedResourceType extends Component {
    ...
    render() {
        const resourceTypeId = (this.props.typeResourceInstances.length > 0 && this.props.typeResourceInstances[0].resource_type_id) || 0;
        ...
        if (this.props.resourceTypes.length > 0) {
            resourceTypeOptions = this.props.resourceTypes.map(rt => (
                <option selected={rt.id === resourceTypeId ? true : false} key={rt.id} value={rt.id}>{rt.resource_type}</option>
            ));
        }
