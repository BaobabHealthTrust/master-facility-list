...
class AdvancedUtilityType extends Component {
  ...
  render() {
    const {
      ...
      utilityTypes,
      handleChange,
      handleChangeAddSearchValue
    } = this.props;

    const utilityTypeId =
      (typeUtilityInstances.length &&
        typeUtilityInstances[0].utility_type_id) ||
      0;
    ...
    if (utilityTypes.length) {
      utilityTypeOptions = utilityTypes.map(ut => (
        <option
          selected={ut.id === utilityTypeId ? true : false}
          key={ut.id}
          value={ut.id}
        >
          {ut.utility_type}
    ...
    if (utilityTypes.length) {
      utilityTypeInstanceOptions = typeUtilityInstances.map(tui => (
        <option key={tui.id} value={tui.id}>
          {tui.utility_name}
        </option>
      ));
    }
