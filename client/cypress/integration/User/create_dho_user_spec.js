...
describe("Adds DHO User", () => {
  ...
  context("Adds DHO User", () => {
    let dhoId = null;
    it("Successfully Adds DHO User", () => {
      cy.server({
        status: 200
      });
      cy.route(
      ...
      cy.fetch_user_groups().then(res => {
        const dhoIndex = res ? res.findIndex(r => r.name === "dho") : -1;

        if (dhoIndex > -1) {
          dhoId = res[dhoIndex].id;
      ...
      cy.get("@add").then(xhr => {
        ...
        cy.expect(data.role).to.equal(dhoId);
      });
    });
  });
});
